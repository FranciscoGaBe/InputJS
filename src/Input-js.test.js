...
describe('InputJS', () => {
  ...
  describe('.joystick', () => {
    ...
    it('passing threshold in options makes the mouse act as joystick', () => {
      ...
      expect(inputjs.joystick.axis.horizontal).toBe(0);
      fireEvent('mouse', 'pointermove', { clientX: 150, clientY: 150 });
      expect(inputjs.joystick.axis.horizontal).toBe(0.5);
    });
    ...
    describe('.vertical', () => {
      it('returns -1 if KeyW or ArrowUp is pressed', () => {
        expect(inputjs.joystick.axis.vertical).toBe(0);
        fireEvent('keyboard', 'keydown', { code: 'KeyW' });
        expect(inputjs.joystick.axis.vertical).toBe(-1);
    ...
    describe('.horizontal', () => {
      it('returns -1 if KeyA or ArrowLeft is pressed', () => {
        ...
        expect(inputjs.joystick.axis.horizontal).toBe(-1);
    ...
    it('has a normalized vector of horizontal and vertical', () => {
      expect(inputjs.joystick.axis.normalized).toEqual({ x: 0, y: 0 });
      fireEvent('keyboard', 'keydown', { code: 'KeyD' });
      fireEvent('keyboard', 'keydown', { code: 'KeyS' });
      expect(inputjs.joystick.axis.normalized).toEqual({ x: 0.71, y: 0.71 });
    });
  });
});
